---
name: unit_conversion
description: >
  Проверка и пересчёт единиц измерения анализов перед клинической интерпретацией.
---

# Unit Conversion — Скилл пересчёта единиц измерения

Иерархия источников правды: см. `v2/HIERARCHY.md`.
Перед выполнением — прочитай RULES.md.

## Назначение

Скилл устраняет ошибки интерпретации из-за разных единиц измерения.

## Стиль формулировок

1. Пояснять пересчёты кратко и понятно, чтобы клинический смысл был прозрачен.
2. При первом упоминании специализированного сокращения давать расшифровку в контексте.

## Входные данные

1. clients/[ФИО]/blood_tests.md и/или исходные бланки
2. references/11_reference_values.md
3. references/unit_conversions.md

## Последовательность

1. Для каждого показателя определить единицу клиента.
2. Сверить с эталонной единицей из references/11_reference_values.md.
3. При несовпадении выполнить пересчёт по references/unit_conversions.md.
4. Зафиксировать исходное и пересчитанное значение.
5. Передать нормализованные значения в skills/analyze_blood.md.

## Выход

Встроенный блок нормализованных показателей в analysis_expert перед клинической интерпретацией.
