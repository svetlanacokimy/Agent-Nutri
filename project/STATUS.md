...
**Протокол входа:** первое действие в новом чате — проверить состояние и доложить пользователю ДО работы: `git status --short` ; `git log --oneline -3` ; поиск двух дефектов в `references\methodology\*.md` — паттерн `\S > 📚` (блок в середине слова) и паттерн `\[EBM:[^\]]*\[EBM:` (вложенные теги). Норма: git status пуст, обе проверки дают ноль. Если что-то найдено — СТОП, не редактировать, доложить.
